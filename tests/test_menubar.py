...
def test_menubar_execute_push_or_summarize_routing():
    """Verify _execute_push_or_summarize calls agent.summarize_and_push when summary_pages > 0."""
    with patch("rumps.Timer"):
        app = QuadernoMenubarApp()

    # Case 1: summary_pages = 3 -> calls agent.summarize_and_push with pages=3
    app.summary_pages = 3
    with patch("quaderno_companion.triggers.menubar.agent.summarize_and_push", new_callable=AsyncMock) as mock_sum, \
        ...
        mock_sum.assert_called_once_with(text_or_url="https://example.com/test", title="Test Page", pages=3)

    # Case 2: summary_pages = 0 -> calls tool_push_document (direct push)
    app.summary_pages = 0
